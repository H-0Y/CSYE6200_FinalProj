    ...
    public void confirmAddStuButton(){
        ...
        try {
            SqlUtil.doSqlWork(mapper -> {
                ...
                result = mapper.addAdvised(student.getSid(), student.getAdvisor().getAid());
                if(result > 0) {
                    log.info("add a new advised info: "+ student);
                }
                else log.info("Failure to add a new advised info: "+ student);
            });
        }catch (Exception e) {
